...
def load_model_weights(model, model_path, device):
    """Load either a plain state_dict or the new checkpoint dict."""
...
def save_comprehensive_vis(
    ...
    for z_idx in range(1, min(20, len(sys_labels)) + 1):
        if sys_labels[z_idx - 1] != invalid_sys_label:
            sys_gt_vol[zones_mask == z_idx] = sys_labels[z_idx - 1]

    if target_gt.sum() > 0:
        best_z = int(np.argmax(target_gt.sum(axis=(1, 2))))
    elif lesion_gt.sum() > 0:
        best_z = int(np.argmax(lesion_gt.sum(axis=(1, 2))))
    elif sys_gt_vol.sum() > 0:
        best_z = int(np.argmax(sys_gt_vol.sum(axis=(1, 2))))
    elif lesion_prob.sum() > 0:
        best_z = int(np.argmax(lesion_prob.sum(axis=(1, 2))))
    else:
        best_z = D // 2
    ...
    axes[0, 2].set_title("Predicted Lesion Risk")
    axes[0, 2].axis("off")
    fig.colorbar(im_risk, ax=axes[0, 2], fraction=0.046, pad=0.04)

    cmap_grade = plt.get_cmap("YlOrRd", 7)
    axes[0, 3].imshow(s_img, cmap="gray")
    grade_masked = np.ma.masked_where(s_grade == 0, s_grade)
    im_grade = axes[0, 3].imshow(grade_masked, cmap=cmap_grade, alpha=0.6, vmin=0, vmax=6)
    axes[0, 3].set_title("Predicted ISUP Grade")
    axes[0, 3].axis("off")
    fig.colorbar(im_grade, ax=axes[0, 3], fraction=0.046, pad=0.04, ticks=range(7))

    axes[1, 0].imshow(s_img, cmap="gray")
    if s_lesion.sum() > 0:
        axes[1, 0].imshow(np.ma.masked_where(s_lesion == 0, s_lesion), cmap="autumn", alpha=0.6)
    axes[1, 0].set_title("GT: Dense Lesion")
    axes[1, 0].axis("off")

    axes[1, 1].imshow(s_img, cmap="gray")
    if s_target.sum() > 0:
        axes[1, 1].imshow(np.ma.masked_where(s_target == 0, s_target), cmap=cmap_grade, alpha=0.8, vmin=0, vmax=6)
    axes[1, 1].set_title("GT: Target Biopsy")
    axes[1, 1].axis("off")

    axes[1, 2].imshow(s_img, cmap="gray")
    if s_sys.sum() > 0:
        axes[1, 2].imshow(np.ma.masked_where(s_sys == 0, s_sys), cmap=cmap_grade, alpha=0.5, vmin=0, vmax=6)
    axes[1, 2].set_title("GT: Systematic Zones")
    axes[1, 2].axis("off")

    axes[1, 3].imshow(s_img, cmap="gray")
    if s_lesion.sum() > 0:
        axes[1, 3].contour(s_lesion, levels=[0.5], colors="green", linewidths=2)
...
def main():
    ...
    # Prefer full checkpoint if present; otherwise fallback to plain model state_dict.
    ckpt_path = os.path.join(TEST_DIR, "best_checkpoint.pth")
    model_path = ckpt_path if os.path.exists(ckpt_path) else os.path.join(TEST_DIR, "best_model.pth")
    ...
    results = []
    invalid_sys_label = int(getattr(Config, "INVALID_SYS_LABEL", -1))
    balanced_evaluator = utils.BalancedAccuracyEvaluator(
        prob_threshold=0.5,
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing & Rendering"):
            ...
            has_gland = batch["has_gland"].numpy()[0]

            grade_logits, sys_grade_preds, lesion_logits, sys_lesion_preds, gland_logits = model(imgs, z_mask)
            lesion_prob_tensor = torch.sigmoid(lesion_logits[0, 0])

            balanced_evaluator.update(
                pred_prob_3d=lesion_prob_tensor,
                ...
                has_sys=bool(has_sys > 0),
                has_lesion=bool(has_lesion > 0),
                has_target=bool(has_target > 0),
            )

            pred_lesion_prob = lesion_prob_tensor.cpu().numpy()
            if has_gland > 0 and gt_gland.sum() > 0:
            ...
            dice_score, f1_score_val, sens_val, kappa_score = np.nan, np.nan, np.nan, np.nan
    ...
    if df_results["Lesion_Dice"].notna().any():
        print("📈 PUB 数据集 (Voxel-level Metrics):")
    ...
    if df_results["Grade_Kappa"].notna().any():
        print("📈 Systematic 数据集 (Region-level Grade):")
        print(f"   - Mean Kappa: {df_results['Grade_Kappa'].mean():.4f}\n")

    print("📈 整体临床评价指标 (Clinical-level BAcc):")
